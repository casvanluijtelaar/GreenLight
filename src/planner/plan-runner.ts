...
async function executeHeuristicStep(
	...
	try {
		switch (step.action) {
			...
			case "select": {
				// Older cached plans stored the chosen option in `step.option`
				// (mirroring the LLM schema field name). Newer plans use `value`.
				// Read both so a cache generated by an older greenlight still replays.
				const selectValue = step.value ?? step.option
				if (!selectValue) throw new Error("select step requires a value")
				if (!step.selector) throw new Error("select step requires a selector")
				const locator = buildLocator(page, step.selector, hintText)
				const tag = await locator.evaluate((el) => el.tagName).catch(() => "")
				if (tag === "SELECT") {
					await locator.selectOption({ label: selectValue })
				} else {
					...
					for (const sel of popupSelectors) {
						...
						try {
							...
							for (let i = 0; i < count && !clicked; i++) {
								...
								const opt = container.getByText(selectValue, { exact: true })
					...
					if (!clicked) {
						...
						for (const role of roles) {
							try {
								const opt = page.getByRole(role, { name: selectValue })
								if (await opt.first().isVisible().catch(() => false)) {
									await opt.first().click()
									clicked = true
									break
					...
					if (!clicked) {
						throw new Error(`Could not find option "${selectValue}" in custom dropdown`)
					}
				}
				break
			}
